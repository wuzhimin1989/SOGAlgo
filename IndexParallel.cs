...
namespace PAT.CSP.LTS
{
    public sealed class IndexParallel : CSPProcess
    {
        public List<CSPProcess> Processes;
        ...
        public IndexParallel(List<CSPProcess> processes)
        {
            Processes = processes;
            ...
            ProcessID = CSPDataStore.DataManager.InitializeProcessID(ProcessID);
        }

        private IndexParallel(List<CSPProcess> processes, HashSet<string>[] alphabets)
        {
            Processes = processes;
            Alphabets = alphabets;
        ...
        public override void MoveOneStep(Valuation GlobalEnv, List<Configuration> list)
        {
            ...
            for (int i = 0; i < Processes.Count; i++)
            {
                //CSPProcess process = Processes[i];
                ...
                for (int j = 0; j < list1.Count; j++)
                {
                    ...
                    if (!Alphabets[i].Contains(evt) || step.IsDataOperation)
                    {
                        ...
                        List<CSPProcess> newProcess = new List<CSPProcess>(Processes.Count);
            ...
            foreach (string evt in barrierEnabledEvents)
            {
                ...
                List<List<CSPProcess>> moves = new List<List<CSPProcess>>();
                moves.Add(new List<CSPProcess>());
                ...
                for (int i = 0; i < Processes.Count; i++)
                {
                    if (Alphabets[i].Contains(evt))
                    {
                        ...
                        List<List<CSPProcess>> toAdd = new List<List<CSPProcess>>(moves.Count);
                        ...
                        foreach (Configuration step in steps)
                        {
                            ...
                            if (!step.IsDataOperation)
                            {
                                if (moves[0].Count == i)
                                {
                                    foreach (List<CSPProcess> list2 in moves)
                                    {
                                ...
                                else
                                {
                                    ...
                                    {
                                        List<CSPProcess> list2 = moves[k];

                                        List<CSPProcess> newProcList = new List<CSPProcess>();
                    ...
                    {
                        foreach (List<CSPProcess> move in moves)
                        {
        ...
        public override HashSet<string> GetAlphabets(Dictionary<string, string> visitedDefinitionRefs)
        {
            if (Processes == null)
            {
                CSPProcess parallelProcess = this.ClearConstant(new Dictionary<string, Expression>());
        ...
        public override List<string> GetGlobalVariables()
        {
            ...
            foreach (CSPProcess var in Processes)
            {
...
//#endif
        ...
        public override CSPProcess ClearConstant(Dictionary<string, Expression> constMapping)
        {
            List<CSPProcess> newnewProcesses = Processes;

            List<CSPProcess> newProceses;
            int size;
            if (Specification.IsParsing)
            {
                ...
                newProceses  = new List<CSPProcess>(size); 
            ...
            for (int i = 0; i < size; i++)
            {
                CSPProcess newProc = newnewProcesses[i].ClearConstant(constMapping);

                if (newProc is IndexParallel && (newProc as IndexParallel).Processes != null)
                {
                    List<CSPProcess> processes = (newProc as IndexParallel).Processes;
        ...
        {
            List<CSPProcess> newnewListProcess = Processes;
            if (Processes == null)
            {
                newnewListProcess = IndexedProcessDefinition.GetIndexedProcesses(constMapping);
            }

            List<CSPProcess> newProceses = new List<CSPProcess>();
            foreach (CSPProcess process in newnewListProcess)
            {
                newProceses.Add(process.Rename(constMapping, newDefNames, renamedProcesses));
            }

            CSPProcess result = new IndexParallel(newProceses);
        ...
        public override void CollectEvent(List<string> allEvents, List<string> calledProcesses)
        {
            ...
            {
                foreach (CSPProcess process in this.Processes)
                {
        ...
        public override CSPProcess GetTopLevelConcurrency(List<string> visitedDef)
        {
            return this;
        }
